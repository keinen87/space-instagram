...
def get_hubble_images_ids(url, collection):
    response = requests.get('{url}{collection}'.format(url=url, collection=collection))
    if response.ok:
    ...
    for image_id in images_ids:
        ...
        if response.ok:
            ...
            filename = os.path.join(
                path,
                '{image_id}.{extension}'.format(image_id=image_id, extension=extension)
                )
...
if __name__ == '__main__':
    ...
    for collection in collections:
        ...
        fetch_hubble_images(hubble_image_url, images_ids, path)
